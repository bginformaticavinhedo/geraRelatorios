...
export default function ApontamentosReport() {
    ...
    const { data, isLoading, isError, error } = useQuery<Apontamento[]>({
        queryKey: ["apontamentos", appliedFilters],
        queryFn: async () => {
            const params = new URLSearchParams();
            if (appliedFilters.startDate) params.append("startDate", appliedFilters.startDate);
            if (appliedFilters.endDate) params.append("endDate", appliedFilters.endDate);

            const res = await fetch(`/api/apontamentos?${params.toString()}`);
            if (!res.ok) {
                let errorData;
                try {
    ...
    const handleExportPDF = () => {
        ...
        exportToPDF(exportData, "Relatório de Apontamentos", ["Title", "Data", "Tecnico", "HoraInicioFormatada", "HoraFinalFormatada", "DuracaoFormatada", "Descricao"]);
    };
    ...
    const handleExportExcel = () => {
        ...
        exportToExcel(exportData, "Relatório de Apontamentos");
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight" style={{ color: '#0d9488' }}>Apontamentos</h2>
                    <p className="text-slate-400 text-sm">Relatório detalhado de horas trabalhadas por técnico.</p>
                </div>
