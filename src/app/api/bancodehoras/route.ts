...
export async function GET(request: Request) {
    ...
    try {
        ...
        let response = await client
            .api(`/sites/${SHAREPOINT_SITE_ID}/lists/Banco de Horas/items`)
            .expand('fields')
        ...
        const items = rawItems.map((item: any) => {
            ...
            if (rawStart && rawEnd) {
                ...
                if (!isNaN(startDate.getTime()) && !isNaN(endDate.getTime())) {
                    ...
                    // Considera que cruzou o almoço se começou antes ou às 12h e terminou depois ou às 13h
                    const crossesLunch = startHour <= 12 && endHour >= 13;
                    
                    // Suporte opcional à coluna SharePoint para forçar o desconto
                    const forceDesconto = fields.DescontarAlmoco === true || fields.DescontarAlmoco === 'Sim' || fields.DescontoAlmoco === true || fields.DescontoAlmoco === 'Sim';
                    const forceSemDesconto = fields.DescontarAlmoco === false || fields.DescontarAlmoco === 'Não' || fields.DescontarAlmoco === 'Nao' || fields.DescontoAlmoco === false || fields.DescontoAlmoco === 'Não';
                    
                    if (!forceSemDesconto) {
                        if (forceDesconto || (isWeekday && crossesLunch)) {
                            // Desconta 1 hora, mas garante que não fique negativo
